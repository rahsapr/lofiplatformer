...
const restartFromDeathButton = document.getElementById('restartFromDeathButton');
const homeFromDeathButton = document.getElementById('homeFromDeathButton');
...
gameOverMusic.loop = false; // Important: ensure it only plays once per death
...
function triggerDeathSequence() {
    ...
    gameOverMusic.play().catch(e => console.error("Game over audio failed:", e));

    gameUI.style.display = 'none';
    deathScreen.style.display = 'flex';
    deathOptions.style.visibility = 'hidden';
...
function checkOtherCollisions() {
    ...
    for(const h of hazards) { if (isColliding(playerRect, h)) { triggerDeathSequence(); return; } }
    if (goal && isColliding(playerRect, goal)) { completeLevel(); return; }
    if (player.pos.y > LEVEL_HEIGHT + 200) { triggerDeathSequence(); }
}
...
// --- Event Listeners ---
window.addEventListener('keydown', (e) => { keys[e.key] = true; if (gameState === 'PLAYING') { if (e.key === ' ' || e.key === 'w' || e.key === 'ArrowUp') { e.preventDefault(); player.jump(); } if (e.key === 'Shift') player.sprint(); if (e.key === 'r' || e.key === 'R') triggerDeathSequence(); } if (gameState === 'GAME_COMPLETE' && e.key === 'Enter') { submitScoreButton.click(); } });
