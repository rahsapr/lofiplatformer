...
function loadSettings() {
    ...
    // Apply loaded settings to UI and audio objects
...
function applyAudioSettings() {
    ...
    // When you have sound effects, set their volume here too.
...
let gameState = 'START'; // States: START, LEVEL_START, PLAYING, END, CONGRATS
...
function initLevel(levelIndex) {
    ...
    const levelMap = LEVEL_MAPS[levelIndex];
    levelMap.forEach((row, rowIndex) => {
        for (let colIndex = 0; colIndex < row.length; colIndex++) {
            ...
            if (tile === 'P') {
                ...
                if (levelMap[rowIndex-1] && levelMap[rowIndex-1][colIndex] === 'S' && !foundStart) {
...
function startGame(levelIndex) {
    ...
    setTimeout(() => {
        if (gameState !== 'LEVEL_START') return; // Don't proceed if user restarted
...
function showLevelCompleteScreen() {
    ...
    if (isLastLevel) {
        levelCompleteTitle.textContent = "You Win!";
        levelCompleteMessage.textContent = "You've beaten all the levels!";
        nextLevelButton.style.display = 'none';
    } else {
        levelCompleteTitle.textContent = `Level ${currentLevelIndex + 1} Complete!`;
        levelCompleteMessage.textContent = "Ready for the next challenge?";
...
window.addEventListener('keydown', (e) => {
    ...
    if (gameState === 'PLAYING') {
        ...
        if (e.key === 'r' || e.key === 'R') startGame(currentLevelIndex);
    }
...
startButton.addEventListener('click', () => {
    if (backgroundMusic.paused) {
        backgroundMusic.play().catch(e => console.error("Audio play failed due to browser policy:", e));
    }
    startGame(0);
});

musicVolumeSlider.addEventListener('input', (e) => {
    gameSettings.musicVolume = e.target.value;
    applyAudioSettings();
    saveSettings();
});
sfxVolumeSlider.addEventListener('input', (e) => {
    gameSettings.sfxVolume = e.target.value;
    saveSettings();
});
masterMuteCheckbox.addEventListener('change', (e) => {
    gameSettings.isMuted = e.target.checked;
    applyAudioSettings();
    saveSettings();
});
